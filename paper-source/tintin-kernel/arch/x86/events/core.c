...
#define EVAL_COUNTER_MINER
int defined_counterminer;
EXPORT_SYMBOL(defined_counterminer);


// #define TINTIN_x86_SCHEDULING_DEBUG
...
// @Tintin's code
	...
	int counterminer_length = 0;
	u64 counterminer_mean = 0;
	u64 counterminer_stddev = 0;
	u64 counterminer_sum = 0;
	u64 counterminer_sum_squares = 0;
	u64 counterminer_mean_squares = 0;
	int counterminer_ii;
	int counterminer_tmp_index;
	u64 sum_squares_div = 0;
#endif
// @Tintin's code
...
again:
	...
	event->delta += delta;
#ifdef EVAL_COUNTER_MINER
if(defined_counterminer == 1){
	if (event->counterminer_count > 10 && !event->attr.pinned) {
		counterminer_length =
			(event->counterminer_max - event->counterminer_min) /
			int_sqrt(event->counterminer_count);
		
		if (counterminer_length > 10000) {
			counterminer_length = 10000;
			printk("[Tintin] Counterminer length is too large, set to 10000\n");
		}

		for (counterminer_ii = event->counterminer_index;
		     counterminer_ii <
		     (event->counterminer_index - counterminer_length);
		     counterminer_ii--) {

			counterminer_tmp_index = event->counterminer_index % 10000;

			counterminer_sum +=
				event->counterminer_data[counterminer_tmp_index];
			counterminer_sum_squares +=
				event->counterminer_data[counterminer_tmp_index] *
				event->counterminer_data[counterminer_tmp_index];
		}

		counterminer_mean = counterminer_sum / counterminer_length;
		sum_squares_div = div64_u64(counterminer_sum_squares, event->counterminer_count);
		counterminer_mean_squares = counterminer_mean * counterminer_mean;
		if (sum_squares_div > counterminer_mean_squares) {
			counterminer_stddev = int_sqrt64(sum_squares_div -
						      counterminer_mean_squares);
		} else {
			counterminer_stddev = int_sqrt64(counterminer_mean_squares -
						      sum_squares_div);
		}
	}

	if (new_raw_count > counterminer_mean + 5 * counterminer_stddev) {
		new_raw_count = counterminer_mean;
		// printk("[Tintin] Counterminer detected a outlier, set to mean\n");
	}

	event->counterminer_data[event->counterminer_index] = new_raw_count;
	event->counterminer_index++;
	if (event->counterminer_index >= 10000) {
		event->counterminer_index = 0;
	}
	event->counterminer_count++;

	if (new_raw_count > event->counterminer_max) {
		event->counterminer_max = new_raw_count;
	}

	if (new_raw_count < event->counterminer_min) {
		event->counterminer_min = new_raw_count;
	}
}
#endif
	// End of Tintin's code
...
#ifdef	TINTIN_x86_SCHEDULING_DEBUG
	printk("[x86] adding events at addr :  0x%p", event);
#endif
...
static void x86_pmu_del(struct perf_event *event, int flags)
{
	...
	if (event->event_tp == TINTIN_EVENT) {
		...
		sc->last_scheduled_out_time = now;
	}
	// End of Tintin's code
	/*
...
static int __init init_hw_perf_events(void)
{
	...
	// Tintin's code
	printk("[Tintin] General HPC number per CPU is %d",
	       x86_pmu.num_counters);
		   printk("[Tintin] Special HPC number per CPU is %d",
			x86_pmu.num_counters_fixed);
	// End of Tintin's code
