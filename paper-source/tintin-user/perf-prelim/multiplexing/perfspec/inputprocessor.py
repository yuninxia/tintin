...
def simple_process_events_arr(all_events_arr, num_of_simultaneous_events):
    ...
    processed_events_arr = []
    combo_of_interest = list(events_combos_list[0])
    event_of_interest = combo_of_interest.pop() # this and the next line make sure the event of interest is placed last
    combo_of_interest.append(event_of_interest)
    event_string = ""
    for index in range(num_of_simultaneous_events - 1):
        event_string += combo_of_interest[index] + ","
    event_string += combo_of_interest[num_of_simultaneous_events - 1]
    processed_events_arr.append(event_string)
