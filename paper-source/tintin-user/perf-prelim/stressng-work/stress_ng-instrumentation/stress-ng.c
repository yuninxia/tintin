...
// Lien Zhu AccPerf ADDITION:
// perf counter syscall wrapper
...
int main(int argc, char **argv, char **envp)
{
	...
	*/
	...
	/*
	 * Lien Zhu AccPerf ADDITION of template code with perf_event_open() calls to configure HPCs 
	 */
    ...
    rc = read(fd, &val1, sizeof(val1));
    assert(rc);
    asm volatile("nop;");  // pseudo-barrier

	printf("Done configuring HPCs with perf_event_open()\n");
	...
	*/
	...
	/*
	 * Lien Zhu AccPerf ADDITION for results printing
	 */
	printf("---------------------------------------- \n");
